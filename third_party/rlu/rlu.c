...
/*
 * Benchmark-harness helpers (efficios-trie-benchmark local patch; see
 * third_party/rlu/PROVENANCE.txt).  Upstream RLU assumes a fixed set of
 * long-lived threads that each call rlu_thread_init() exactly once, because
 * uniq_id is drawn from the monotonic g_rlu_cur_threads counter that is never
 * decremented.  Our list-scaling harness instead spawns a FRESH set of worker
 * threads for every sweep point, so without a way to recycle the registry the
 * counter would overrun RLU_MAX_THREADS after a couple of points.
 *
 * rlu_bench_reset_threads() rewinds the registry to empty; the caller MUST
 * invoke it only when no RLU thread is live (between sweep points, after every
 * worker has joined and flushed).  rlu_init_quiescence() already skips NULL
 * slots, so clearing the array is sufficient.  rlu_bench_flush() forces a
 * thread's own deferred write-sets to sync+writeback (same as the flush half of
 * rlu_thread_finish, without the stats accumulation) so no object is left
 * locked by a thread that is about to disappear from the registry.
 */
void rlu_bench_reset_threads(void) {
	long th_id;

	for (th_id = 0; th_id < g_rlu_cur_threads; th_id++)
		g_rlu_threads[th_id] = NULL;
	g_rlu_cur_threads = 0;
	MEMBARSTLD();
}

void rlu_bench_flush(rlu_thread_data_t *self) {
	rlu_sync_and_writeback(self);
	rlu_sync_and_writeback(self);
}
