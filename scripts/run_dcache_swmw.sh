...
#
# Three arms, one binary each (built by `make -C experiments/dcache
# bench_dcache_dlm bench_dcache_dlm_swmw bench_dcache_dlm_swmw_pad`):
#   dlm       -- default: index SW (bucket-locked) + per-host transition CHAIN LOCK
#                (sizeof(dentry) 176)
#   dlm-swmw  -- -DDC_CHAIN_SWMW: the chain (d_fwd/d_back) rides the mixed commit as
#                MW records; chain lock RETIRED (sizeof 168)
#   dlm-swmw-pad -- -DDC_CHAIN_SWMW -DDC_SWMW_PAD: the mixed engine with the retired
#                lock's 8 bytes restored as dead padding (sizeof 176), the SAME-SIZE
...
declare -A BINOF=( [dlm]=bench_dcache_dlm \
                   [dlm-swmw]=bench_dcache_dlm_swmw \
                   [dlm-swmw-pad]=bench_dcache_dlm_swmw_pad )
ENGINES="dlm dlm-swmw dlm-swmw-pad"
for e in $ENGINES; do
