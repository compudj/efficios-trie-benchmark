...
/* LOCAL PATCH (efficios-trie-benchmark): wrapped in #ifndef so the boundary is
 * -D overridable.  Upstream hardcodes 1214, measured on THEIR 224-core Intel
 * box.  The value is machine-specific (see tools/ordo upstream) and MUST be
 * re-measured on any host before the ordo variant's numbers mean anything.
 * Same convention as third_party/rlu's -D-overridable tunables. */
#ifndef __ORDO_BOUNDARY
