...
template<typename ValueType, template <typename> typename KeyExtractor> struct HOTRowex {
	...
	/* efficios-trie-benchmark: dropped `const` so the (unused) move-assignment
	 * operator instantiates under GCC.  The pointer is set in the ctor and only
	 * ever read via MemoryGuard, so this is behaviourally inert. */
