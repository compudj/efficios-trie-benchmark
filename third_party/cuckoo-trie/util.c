...
void* mmap_hugepage(size_t size) {
	...
	if (result == MAP_FAILED) {
		/*
		 * efficios-trie-benchmark local change: when reserved 2 MiB
		 * hugepages (MAP_HUGETLB) are unavailable (nr_hugepages == 0 and
		 * no root to reserve them), fall back to a regular anonymous
		 * mmap + MADV_HUGEPAGE so the trie can still be built.  This is
		 * THP-backed rather than guaranteed hugetlb, a slight deviation
		 * from upstream Cuckoo Trie's design — note it when interpreting
		 * results.  (Upstream aborts here.)
		 */
		result = mmap(0, size, PROT_READ | PROT_WRITE,
			      MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
		if (result == MAP_FAILED) {
			printf("Failed to allocate %lu bytes (MAP_HUGETLB and plain mmap both failed)\n", size);
			return NULL;
		}
#ifdef MADV_HUGEPAGE
