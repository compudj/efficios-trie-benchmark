...
/*
 * VENDORED from the Linux kernel: kernel/locking/rwsem.c (Linux 7.0.0).
 * Unmodified except for this include block: the kernel headers are replaced by
 * "kcompat.h", a userspace shim (see that file).  Built with
 * CONFIG_RWSEM_SPIN_ON_OWNER=n / CONFIG_PREEMPT_RT=n / CONFIG_DEBUG_RWSEMS=n,
 * so only the fairness slow-path compiles -- the fairness behaviour is the
 * kernel's, verbatim.  GPL-2.0 (see the SPDX line above).
 */
#include "kcompat.h"
