...
/* Local patch (efficios-trie-benchmark): #ifndef-guarded so the caps are
 * -D overridable without touching upstream logic.  The bidir-list sweep goes
 * to 192 threads (>128), and our commits touch only 2-3 tiny nodes, so the
 * default 20 MB/thread of write-set buffers is wasteful.  See PROVENANCE.txt. */
#ifndef RLU_MAX_THREADS
...
#ifndef RLU_MAX_WRITE_SET_BUFFER_SIZE
...
/* efficios-trie-benchmark local patch (see PROVENANCE.txt): recycle the thread
 * registry between sweep points (call only when no RLU thread is live), and
 * flush a thread's own deferred write-sets before it leaves the registry. */
